...
for (const demo of demos) for (const lang of languages) {
  ...
  const slides = demo.slides.map((_, index) => `<${demo.presentationComponent || 'PitchSlide'} demo="${demo.id}" locale="${lang}" :index="${index}" />`).join('\n\n---\n\n')
  const layout = demo.presentationComponent ? front.replace('layout: default', 'layout: none') : front
  writeFileSync(new URL(`../presentations/${demo.id}.${lang}.md`, import.meta.url), layout + '\n' + slides + '\n')
}
